...
BEGIN
  ...
  IF NEW.rfc IS NOT NULL AND NEW.rfc != '***ENCRYPTED***' THEN
    NEW.rfc_encrypted := encrypt_sensitive_data(NEW.rfc);
    NEW.rfc := '***ENCRYPTED***';
  END IF;
  
  -- Encriptar razón social si existe
  IF NEW.razon_social IS NOT NULL AND NEW.razon_social != '***ENCRYPTED***' THEN
  ...
  IF NEW.direccion_fiscal IS NOT NULL AND NEW.direccion_fiscal != '***ENCRYPTED***' THEN
...
DROP POLICY IF EXISTS "Solo usuarios verificados pueden crear empresa" ON public.empresas;
...
DROP POLICY IF EXISTS "Solo admin empresa puede actualizar su empresa" ON public.empresas;
...
DROP POLICY IF EXISTS "Solo admins pueden ver auditoría de empresas" ON public.auditoria_acceso_empresas;
...
DROP POLICY IF EXISTS "Sistema puede insertar auditoría" ON public.auditoria_acceso_empresas;
