...
interface MarketplaceReclutadorProps {
  reclutadorUserId?: string;
}

export const MarketplaceReclutador = ({ reclutadorUserId }: MarketplaceReclutadorProps) => {
  ...
  if (loading) {
    return <div className="animate-pulse">Cargando tus vacantes publicadas...</div>;
  }
      ...
      {filteredPublicaciones.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground">
          {searchTerm 
            ...
            : "No tienes vacantes publicadas en el marketplace"}
        </div>
      ) : (
